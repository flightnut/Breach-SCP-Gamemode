...
function ulx.explode( calling_ply, target_plys )
	...
	if #target_plys > 1 then
		print('[ULX Explode] '..calling_ply:GetName().. " exploded: ")
		for k,v in pairs(target_plys) do
			print('\t-'..v:GetName())
		end
	else
		print('[ULX Explode] '..calling_ply:GetName().. " exploded: "..target_plys[1]:GetName())
	end
...
function ulx.ammo( calling_ply, target_plys, amount, should_setammo )
	...
	for k,ply in pairs(target_plys) do
		...
		for k,actwep in pairs(plyweps) do
			if IsValid(actwep) then
				if actwep:GetClass() ~= "weapon_sh_flashbang" and actwep:GetClass() ~= "weapon_sh_cryogrenade" then
					if actwep.GetPrimaryAmmoType then
						local curammo = actwep:GetPrimaryAmmoType()
						if not table.HasValue(plyammos,curammo) then
							table.ForceInsert(plyammos,curammo)
						if not should_setammo then
								ply:GiveAmmo( amount, curammo )
							else
							ply:SetAmmo( amount, curammo )
							end
						end
					end
				end
