...
function ulx.administrate( calling_ply, force_revoke )

	if force_revoke then
		should_revoke = true--If they are in admin mode make it togglable
		calling_ply.AdminMode = false
	else
		if calling_ply.AdminMode then
			calling_ply.AdminMode = false
		else
			calling_ply.AdminMode = true
		end
		should_revoke = (not calling_ply.AdminMode) --If they are in admin mode make it togglable
	end
...
administrate:setOpposite( "ulx unadministrate", { _, true }, {"!unadministrate","!unadmin"}, true )
