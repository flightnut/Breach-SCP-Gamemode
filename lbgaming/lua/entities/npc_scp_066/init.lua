...
-----------------------------------------------*/
ENT.Model = {"models/cpthazama/scp/vj_scp066.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = GetConVarNumber("vj_scp_066_h")
ENT.HullType = HULL_TINY
---------------------------------------------------------------------------------------------------------------------------------------------
ENT.GodMode = true -- Immune to everything
ENT.RunOnTouch = false -- Runs away when something touches it
ENT.PlayFearSoundOnTouch = false -- Should it play a sound when something touches it?
...
-- Custom
ENT.SCP066_NextChaseTime = 0
ENT.SCP066_NextCloseSoundTime = 0
ENT.SCP066_BeethovenActivated = false
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:CustomOnThink_AIEnabled()
	if self.SCP066_BeethovenActivated == true then
	...
	for k,v in ipairs(findents) do
		if v:IsPlayer() then
			if v:Team() ~= TEAM_SCP and v:Team() ~= TEAM_SPEC then --Link2006's to skip if they are a spectator or SCP
				if self:GetPos():Distance(v:GetPos()) <= 50 && CurTime() > self.SCP066_NextCloseSoundTime then
					self:StopMoving()
					if math.random(1,15) == 1 then
						self.SCP066_BeethovenActivated = true
						self:AlertSoundCode({"vj_scp066/Beethoven.wav"})
						self.NextIdleTime = CurTime() + math.random(11,12)
						self.SCP066_NextCloseSoundTime = CurTime() + 26
						self.SCP066_NextChaseTime = CurTime() + 11
						timer.Simple(20,function() if IsValid(self) then self.SCP066_BeethovenActivated = false end end)
						//util.ScreenShake(self:GetPos(),10,10,23,100)
					else
						self.SCP066_BeethovenActivated = false
						...
						self.SCP066_NextCloseSoundTime = CurTime() + 5
					end
				end
				if CurTime() > self.SCP066_NextChaseTime then
					self:SetLastPosition(v:GetPos())
					self:SetSchedule(SCHED_FORCED_GO_RUN)
					self.SCP066_NextChaseTime = CurTime() + 5
				end
			end
		end
	end
