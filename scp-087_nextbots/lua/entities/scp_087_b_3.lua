...
--SCP-087-B-3 Coded by me Yay! Don't download if you don't like Horror Games! :-)

ENT.Base 				=	"base_nextbot"
ENT.Spawnable			=	false

CreateConVar( "087b3_chatmassage", 0, FCVAR_NOTIFY, FCVAR_ARCHIVE, "Enable or disable the chatbox massages when killed by SCP-087-B-3! [Default '0']" )
CreateConVar( "087b3_doorprops", 1, FCVAR_NOTIFY, FCVAR_ARCHIVE, "If set to 1 doors will create props after beeing destroyed. If set to 0 they just get removed by SCP-087-B-1! [Default '1']" )
CreateConVar( "087b3_tp", 1, FCVAR_NOTIFY, FCVAR_ARCHIVE, "Enable or disable SCP-087-B-3's teleporting. [Default '1']" )
CreateConVar( "087b3_health", 4000, FCVAR_NOTIFY, FCVAR_ARCHIVE, "SCP-087-B-3's health. [Default '8000']" )
CreateConVar( "087b3_speed", 402, FCVAR_NOTIFY, FCVAR_ARCHIVE, "SCP-087-B-3's speed. [Default '500']" )

util.PrecacheSound( "sound/gman/doorbreak.wav" )

if CLIENT then
language.Add( "scp_087_b_3", "SCP-087-B-3" )
end

function ENT:Initialize()
local hlth = GetConVar( "087b3_health" )

	self.Oeightsevenambience = CreateSound(self,"087_ambient.wav")
	self:SetModel( "models/scp/scp_087_b_3.mdl" )
...
function ENT:FindEnemy() --Currently gets the first valid enemy it finds...
	...
	for k,v in pairs(player.GetAll()) do -- Go Through Everyone, until there's no players to select from. 
		...
		if _newTarget:Team() ~= TEAM_SCP and _newTarget:Team() ~= TEAM_SPEC and _newTarget:Alive() then --Not SCP, Not Spectator, Still Alive? GO! :)
			...
			self:EmitSound("0873_alert_" .. math.random(1, 2) .. ".wav", 100, 100) --Play a sound 
...
function ENT:RunBehaviour()
	while ( true ) do
		if ( self:HaveEnemy() ) then
			if ( GetConVar( "087b3_speed" ):GetFloat() <= 200 ) then
				self:StartActivity( ACT_WALK )
				self.loco:SetDesiredSpeed( GetConVar( "087b3_speed" ):GetFloat() )
...
function ENT:ChaseEnemy( options )
	...
	local gcvar = GetConVar( "087b3_doorprops" )
	local gcvar2 = GetConVar( "087b3_tp" )
	local massage = GetConVar( "087b3_chatmassage" )
							...
							for k, ply in pairs( player.GetAll() ) do
								if massage:GetFloat() == 1 then do
									if enemy:GetName() == "" then
										...
										ply:ChatPrint("SCP-087-B-3 Caught another soul!")
...
--local gcvar = GetConVar( "087b3_chatmassage" )
--
	--if ent:IsPlayer() || ent:IsNPC() then
		--if ent:Health() > 0 then do
	--ent:TakeDamage( 20, self, ent ) end
	--if gcvar:GetFloat() == 1 then do
...
function ENT:Teleport()
	...
	self:EmitSound("0873_alert_" .. math.random(1, 2) .. ".wav", 100, 100)
end
