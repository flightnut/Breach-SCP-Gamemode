...
hook.Add("PlayerTraceAttack", "PlayerTraceAttack_SpecDM", function(ply, dmginfo, dir, trace)
	if ply:IsGhost() then
		...
		if hs then
			...
			local wep = GetWeaponFromDamage(_dmginfo)
			if IsValid(wep) then
...
hook.Add("Initialize", "Initialize_SpecDM", function()
	...
	--HACK: PORT TO BREACH'S KARMA SYSTEM

	function meta:ReduceKarma(amount)
		if KarmaEnabled() == false then return end --Karma's disabled, don't touch.
		if self:IsGhost() then return end --We're in SpecDM, Don't lower karma.
