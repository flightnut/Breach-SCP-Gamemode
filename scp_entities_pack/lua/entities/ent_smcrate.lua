...
local BG = vgui.Create( "DImage",DermaPanel)
		...
		BG:SetSize( 350, 70 ) -- Set the size of the panel
...
net.Receive("sm_Refresh_Crate_Menu", function(len)
...
function ENT:SpawnFunction( Player, Trace )
	...
	if SERVER then
	local Entity = ents.Create( "ent_smcrate" );
...
if SERVER then
util.AddNetworkString("sm_Open_Crate_Menu")
util.AddNetworkString("sm_Refresh_Crate_Menu")
util.AddNetworkString("sm_Crate_Spawn_Item")
AddCSLuaFile("ent_smcrate.lua")
net.Receive("sm_Crate_Spawn_Item", function(len,ply)
...
if ent:GetNWBool("CanBePlaced") and ent:BoundingRadius() < 25 and #self.Contents < 5 then
...
local count = #ents.FindByClass("ent_smcrate")
if count > 0 then
...
for _,ent in pairs(ents.FindByClass("ent_smcrate"))do
